...
private:
    ...
    int remote_mode;    // control remote self

public:
    friend class Tv;
    enum {Normal, Interactive};     // 模式

public:
    Remote(int m = TV, int rm = Normal) : mode(m), remote_mode(rm) {}
    bool volup(Tv & t);
    bool voldown(Tv & t);
    void onoff(Tv & t);
    void chanup(Tv & t);
...
public:
    ...
    friend void Remote::set_remote_mode();
    ...
    bool set_remote_mode(Remote & remote) 
    { 
        if(remote.remote_mode == Remote::Interactive)
        {
            remote.remote_mode = Remote::Normal;
            return true;
        }
        else
            return false;
...
inline void Remote::set_remote_mode() { remote_mode = (remote_mode == Normal) ? Interactive : Normal; }
inline void Remote::show_remote_mode() const { std::cout << "Remote mode: " << remote_mode << std::endl; }
